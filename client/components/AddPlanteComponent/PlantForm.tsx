...
export default function PlantForm() {
    ...
    const [humidityRateSensors, setHumidityRateSensors] = useState<{ id: string; name: string }[]>([]);

    // Récupération des capteurs depuis l'API
    useEffect(() => {
        const fetchSensors = async () => {
            try {
                ...
                setHumidityRateSensors(data); // Assurez-vous que `data` est un tableau d'objets avec `id` et `name`
            } catch (error) {
                console.error("Erreur lors de la récupération des capteurs :", error);
                alert("Impossible de récupérer les capteurs. Veuillez vérifier le serveur.");
            }
        };
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        ...
        formData.append("humidityRateSensorId", data.humidityRateSensorId);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} style={styles.formStyle} id="formData">
            ...
            {/* Champ : Capteur d'humidité */}
            <label className="flex flex-col" style={styles.labelStyle}>
                <ThemedText>
                    <span className="font-medium">Capteur d'humidité :</span>
                </ThemedText>
                <select
                    id="humidityRateSensorId"
                    style={styles.inputStyle}
                    {...register("humidityRateSensorId", {required: "Veuillez sélectionner un capteur."})}
                    className="border p-2 rounded"
                >
                    {humidityRateSensors.map((humidityRateSensor) => (
                        <option key={humidityRateSensor.id} value={humidityRateSensor.id}>
                            {humidityRateSensor.name}
                        </option>
                    ))}
                </select>
                {errors.humidityRateSensorId && (
                    <span className="text-red-500 text-sm" style={styles.warningStyle}>
                        {errors.humidityRateSensorId.message}
                    </span>
                )}
            </label>
